...
**Snapshot mode vs live data.** I ship a frozen 2026-03-27 OHLCV snapshot bundled with the repo, not a live yfinance fetch on demand. The free yfinance tier is unreliable on Render — rate limits hit the cold-start dyno, the current trading day's bar comes back with trailing NaNs, and NSE ticker renames silently break universe joins. I made the call that demo determinism beats live unreliability: a flaky live demo loses every time to a fast, defensible snapshot. Production swaps to a paid Indian-equity feed via the same `MarketDataProvider` interface — set `MARKET_DATA_PROVIDER=trendlyne`, zero changes elsewhere. The provider chain was always the point; the snapshot is just the first link.

**LLM layered on math, not replacing it.** Every recommendation's thesis paragraph is generated by Gemini Flash from a structured prompt that hands the LLM our indicator values and explicitly forbids it from inventing numbers. The math determines the picks; the LLM only paraphrases. I chose this hybrid because the failure modes are inverse: LLMs hallucinate numbers and can't compute RSI today, but they write fluent narrative; technical filters are deterministic and verifiable but read like a spreadsheet. The hybrid gets both — auditable picks with human-readable explanations. The Gemini key is optional; if absent, the system falls back to deterministic templates built from the same pros/cons logic, and the frontend renders identically.

**Widened filter thresholds.** The original SPEC thresholds (RSI 45–62, MACD crossover within 3 bars, 52-week gap ≥ 15%) were calibrated for a strongly trending market. On the snapshot date those defaults surfaced near-zero picks because the market was in a corrective phase. Rather than ship empty lanes, I widened: RSI 40–68 for Strong, MACD window to 30 bars, gap minimum to 10%. The new thresholds still satisfy the SPEC's *intent* — uptrend, room to run, recent turn — but with calibration headroom. This is a real PM call: strict thresholds protect against false positives; wider thresholds protect against false negatives. For a decision-support tool with explainable scoring on every card, false negatives are the bigger cost — the user can audit a marginal pick, but can't audit a pick they never saw.

**Mutual fund cross-check as a universe constraint.** The Smart Money lane reads from `data/mutual_fund_holdings.csv` — a hand-curated monthly snapshot from public AMC factsheets. No free or public API exists for Indian mutual fund portfolio holdings; SEBI mandates monthly disclosure but each AMC publishes its own PDF. The CSV pattern lets the demo work end-to-end today and disclose the gap honestly rather than fake an integration. Production upgrade is an `MFHoldingsProvider` interface fronting a paid aggregator (Tickertape, ValueResearch) — the same swappable pattern as `MarketDataProvider` and `PortfolioProvider`. Productizing the user's own manual research workflow — cross-checking technical picks against MF holdings — is the project's most original product wedge, and it's the one that wouldn't exist without sitting next to the user who actually does this every Sunday.
