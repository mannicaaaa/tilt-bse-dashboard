// Layout chrome: Sidebar + Topbar + Shell
...
const Sidebar = ({ current, onNavigate }) => {
  ...
  return (
    ...
    >
      {/* Logo spot — sits in the topbar zone but inside sidebar so expansion can cover label */}
      <div className="h-14 flex items-center gap-2.5 px-4 border-b border-line shrink-0">
        ...
        <div
          className="text-[15px] font-semibold tracking-tight text-fg whitespace-nowrap transition-opacity duration-150"
      ...
      <div className="px-3 py-3 border-t border-line text-[11px] text-fg-faint whitespace-nowrap">
        <div style={{ opacity: expanded ? 1 : 0 }} className="transition-opacity duration-150">
          v0.4.2 · localhost
        </div>
      </div>
    </div>
...
const Topbar = ({ lastRefreshed, refreshing, onRefresh, theme, onToggleTheme, current }) => {
  const titles = {
    dashboard: 'Dashboard',
    portfolio: 'Portfolio',
    scan: 'Rally Scanner',
    ...
    <div className="fixed top-0 right-0 z-30 h-14 border-b border-line bg-ink-900/85 backdrop-blur" style={{ left: 56 }}>
      <div className="h-full px-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h1 className="text-[14px] font-semibold text-fg tracking-tight">{titles[current] || 'Tilt'}</h1>
          <span className="hidden sm:inline-flex items-center gap-1.5 px-2 h-6 rounded border border-line bg-ink-700 text-[11px] text-fg-muted">
            <span className="w-1.5 h-1.5 rounded-full bg-bull tilt-pulse"></span>
            <span className="mono">NSE · LIVE</span>
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div className="hidden md:flex items-center gap-2 text-[12px] text-fg-muted mono">
            <Icon.Clock size={13} />
          ...
          <Button
            variant="primary"
            size="md"
            icon={<Icon.RefreshCw size={14} className={refreshing ? 'tilt-spin' : ''} />}
            onClick={onRefresh}
            disabled={refreshing}
          >
...
const Shell = ({ children }) => (
  <div className="min-h-screen pl-14 pt-14">
    <div className="mx-auto px-8 py-8" style={{ maxWidth: 1440 }}>
      {children}
    </div>
  </div>
);

// Page header used inside every screen
const PageHeader = ({ title, subtitle, right, kicker }) => (
  <div className="flex items-start justify-between gap-6 mb-6">
    <div className="min-w-0">
      {kicker && (
        <div className="text-[11px] uppercase tracking-[0.14em] text-fg-dim font-medium mb-2">{kicker}</div>
      )}
...
const SectionLabel = ({ children, right }) => (
  <div className="flex items-center justify-between mb-3">
    <div className="text-[11px] uppercase tracking-[0.14em] text-fg-dim font-semibold">{children}</div>
    {right}
  </div>
);

Object.assign(window, { Sidebar, Topbar, Shell, PageHeader, SectionLabel, NAV_ITEMS });
